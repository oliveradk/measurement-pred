...
def load_model(model_type: ModelTypes, pretrained_model_name: str):
    if model_type in (ModelTypes.CODEGEN, ModelTypes.CODEGEN.value):
        ...
        config = CodeGenMeasurementPredictorConfig(**filter_config(AutoConfig.from_pretrained(pretrained_model_name).to_dict()))
        ...
        config = GPTNeoXMeasurementPredictorConfig(**filter_config(AutoConfig.from_pretrained(pretrained_model_name).to_dict()))
